// lorc's "rock" from game-icons.net (CC BY 3.0) — react-icons only exposes one
// GiRock (delapouite's), so we inline this one. Drop-in for the icon registry.
interface Props {
  size?: number | string;
  color?: string;
}

export default function RockIcon({ size = 16, color = "currentColor" }: Props) {
  return (
    <svg viewBox="0 0 512 512" width={size} height={size} fill={color} aria-hidden="true">
