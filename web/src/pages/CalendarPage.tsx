...
// One metric row in the totals column. The icon keeps its color always; only the
// value text dims when it's zero.
function TotalRow({
	icon: Icon,
	color,
	value,
	...
	return (
		<Group gap={5} wrap="nowrap">
			<Icon size={13} stroke={1.8} color={color} />
			<Text size="xs" c={zero ? "dimmed" : undefined} fw={600}>
				{value}
			</Text>
		</Group>
	);
...
export default function CalendarPage() {
	...
	const RunIcon = categoryIcon.running;
					...
					{weeks.map((w) => {
						...
						return (
							<div className="week-col-cell" key={w.format("YYYY-MM-DD")}>
								<Stack gap={4}>
									{/* Running: two totals stacked in a column beside the icon */}
